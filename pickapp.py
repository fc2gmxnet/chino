...
if len(items) > max_per_row:
    # First row
    cols1 = st.columns(max_per_row)
    for i, (label, url) in enumerate(items[:max_per_row]):
        with cols1[i]:
            st.markdown(
                f"""
    ...
    # Second row
    cols2 = st.columns(len(items) - max_per_row)
    for j, (label, url) in enumerate(items[max_per_row:]):
        with cols2[j]:
            st.markdown(
                f"""
                <a href="{url}" target="_blank">
